<template>
    ...
    <div class="box">
        ...
        <!-- <div class="logo">作业界面logo</div> -->
        <div class="overblack" @click="backTop">^</div>
        <div class="centent">
                <el-button type="text" @click="dialogVisible = true" style="padding:10px;color:black;color:#fff" :class="{test_info : start != 1 }" >提交作业</el-button>
                        ...
                        <div class="topicsType" v-if="topics.topic_content.length != 0 ">
                            ...
                            <div class="topic-content" v-for="(t, index) in topics.topic_content" :key="index" style="margin-left:20px;margin-bottom: 10px">
                                ...
                                <div class="radio" v-if="topics.topicType == 1" >
                                  ...
                                  <el-radio v-for="(item,index) in t.substr(t.lastIndexOf('|')+1).split(',')" :key="index" v-model="lest[t.substr(0,t.indexOf('|'))]"
                                ...
                                <div class="checkbox" v-if="topics.topicType == 2">
                                  <el-checkbox-group v-model="checkList">
                                    <el-checkbox :label="t.substr(0,t.indexOf('|'))+item" v-for="(item,index) in t.substr(t.lastIndexOf('|')+1).split(',')" 
                                    :key="index" 
                                ...
                                <div class="fillInBlank" v-if="topics.topicType == 3">        
                                   <div v-for="(item,indexs) in t.substr(t.lastIndexOf('|')+1).split(',')" :key="indexs" @change="completion(index,t.substr(0,t.indexOf('|')),topics.topicType)">
                                       <!-- <div>{{item}}</div> -->
                                       <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="lest[t.substr(0,t.indexOf('|'))]" style="width:400px;height:100px;line-heigth:100px;" ref="br" class="input_box"></el-input>
                                        <!-- <input  type="textarea" placeholder="请输入内容" style="width:400px;height:100px;line-heigth:100px;" ref="test" class="input_box"> -->
                                      </div>
                                  <!-- {{ t.userAnswer }} -->
                                </div>
                                ...
                                <div class="text" v-if="topics.topicType == 4">
                                    <div v-for="(item,index) in t.substr(t.lastIndexOf('|')+1).split(',')" :key="index"  @change="describe(index,t.substr(0,t.indexOf('|')),topics.topicType)">
                                        <!-- <div>{{item}}</div> -->
                                        <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="lest[t.substr(0,t.indexOf('|'))]" style="width:400px;height:100px;line-heigth:100px;" ref="br" class="input_box"></el-input>
                                         <!-- <input  type="textarea" placeholder="请输入内容" style="width:400px;height:100px;line-heigth:100px;" ref="des" class="input_box"> -->
                                    </div>
                                </div>
                                 <!-- 简答题 -->
                                <div class="texts" v-if="topics.topicType == 5">
                                    <div v-for="(item,index) in t.substr(t.lastIndexOf('|')+1).split(',')" :key="index" @change="brief(index,t.substr(0,t.indexOf('|')),topics.topicType)">
                                        <!-- <div>{{item}}</div> -->
                                        <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="lest[t.substr(0,t.indexOf('|'))]" style="width:400px;height:100px;line-heigth:100px;" ref="br" class="input_box"></el-input>
                                       <!-- <input  type="textarea" placeholder="请输入内容" style="width:400px;height:100px;line-heigth:100px;" ref="br" class="input_box"> -->
                                    </div>
                                </div>
                            </div>                                                                                                                                                         
                      </div>                                                                                                                                                                                                                   
...
export default {
   ...
   mounted(){  
    //    console.log(this.$route.query.test_key,this.$route.query.course_id)
            this.$http.get('/task/viewhomework',{'test_key':this.$route.query.test_key, 'course_id':this.$route.query.course_id})
            .then(
                res => {
                    ...
                    if(res.code == 200){
                         res.data.forEach((element,key)=> {
                             for (let item of this.sortedTopics) {
                                 if( element.type == item.topicType){
                                     item.topic_content.push(element.content)
                                    console.log(element.content)
                                     if(element.content.substr(element.content.indexOf('(')+1,1) == 1 ){//判断是不是单选 走不同的model
                                                this.$set(this.lest,element.content.substr(0,element.content.indexOf('|')),element.content.substring(element.content.lastIndexOf('|')+1,element.indexOf('(')))
                                            }else if(element.content.substr(element.content.indexOf('(')+1,1) == 2){//判断是否是多选 走不同的model
                                                this.$set(this.lest, element.content.substr(0,element.content.indexOf('|')), '');                                         
                                            }else {
                                                this.$set(this.lest, element.content.substr(0,element.content.indexOf('|')), '');       
                                            }
                                 }
        ...
        submt() {
            ...
            this.$http.get('/home/studentdesc')
                ...
                this.ccw.forEach((item )=> {
                   ...
                   item.deal_text.forEach((once,key)=> {
                       newduo += once.trim()
                   })
                   let a = -1
